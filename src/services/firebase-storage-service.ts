/**
    ...
    export const googleSignInStorage = async (): Promise<void> => {
      await signInWithRedirect(fbAuth, gProvider);
      // Page will redirect to Google — result handled in handleStorageRedirectResult on return
    };

    export const handleStorageRedirectResult = async (): Promise<User | null> => {
      try {
        ...
        console.error('Storage redirect result error:', err);
        throw err;
      }
      return null;
    };
