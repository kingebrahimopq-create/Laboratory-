...
export interface IDatabaseAdapter {
  ...
  isConnected(): boolean;
  sync?(remoteDb: IDatabaseAdapter): Promise<SyncResult>;
...
export class DatabaseAdapter {
  ...
  static create(type?: StorageType): IDatabaseAdapter {
    const targetType = type || this.config.type;
    if (targetType === StorageType.SUPABASE && this.config.url && this.config.key) {
