...
export function AdminPanel({ 
  ...
  const handleDelegateOwnership = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setDelegationEmail('');
      await addAuditLog({
        userId: supabaseAuth.currentUser?.uid || 'unknown',
        username: supabaseAuth.currentUser?.email || 'unknown',
  ...
  const loadAllAdminData = async () => {
    try {
      ...
      setQcLogs(allQC.sort((a,b) => getTime(b.createdAt) - getTime(a.createdAt)));
      setAuditLogs(allAudits.sort((a,b) => getTime(b.createdAt) - getTime(a.createdAt)));
      setTestsCatalog(catalog);
      
      const sortedAppointments = allAppointments.sort((a, b) => getTime(b.createdAt) - getTime(a.createdAt));
