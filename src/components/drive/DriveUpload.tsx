...
export function DriveUpload() {
  ...
  const [storageTab, setStorageTab] = useState<'firebase' | 'local'>('firebase');
  ...
  const handleFirebaseUpload = async () => {
    ...
    try {
      ...
      alert('فشل رفع الملف إلى سحابة Firebase. يرجى مراجعة الصلاحيات الأمنية للـ Storage.');
    } finally {
      setUploading(false);
    }
  };

  // 2. Google Drive Upload Handler (Removed)
